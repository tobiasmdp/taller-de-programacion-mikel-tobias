...
public class ControladorPromocionTemporalNuevo implements ActionListener, Observer{
	private Local modelo;
	private VentanaPromocionTemporalNuevo vista;
	private boolean dosx1;
	private boolean descuentoxcantidad;
	
	public ControladorPromocionTemporalNuevo() {

		this.modelo = Local.getInstance(); // arranca el modelo
		this.modelo.addObserver(this);
		
		this.vista = new VentanaPromocionTemporalNuevo(); // arranca la vista
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (comando.equals("CREAR")) {
			...
			if (this.vista.getTxtAcumulable().getText().equalsIgnoreCase("si") )
					...
					acumulable=false;
			MetodosFacturacion.getInstance().altaPromocionTemporal(this.vista.getTxtNombre().getText(),formaDePago,porcentaje,this.vista.getTxtDiaPromo().getText(),activa,acumulable);
		}
